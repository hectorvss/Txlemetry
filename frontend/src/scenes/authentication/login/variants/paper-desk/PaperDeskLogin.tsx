...
function Login(): JSX.Element {
    ...
    return (
        <PaperDeskScene notes={['// welcome back', '// product signals in motion']}>
            {preflight?.cloud && <RedirectIfLoggedInOtherInstance />}
            <PaperDeskCard footer={footer}>
                <CardTitle
                    title={isEmailVerificationSent ? 'Check your email' : 'Log in to Txlemetry'}
                    sub={isEmailVerificationSent ? undefined : 'Welcome back to your product intelligence workspace.'}
                />
                {generalError && (
                    <div
                        className={twMerge(
