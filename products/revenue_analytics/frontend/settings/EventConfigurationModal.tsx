...
export function EventConfigurationModal({ event, onClose }: EventConfigurationModalProps): JSX.Element | null {
    ...
    const accessDeniedReason = getAccessControlDisabledReason(
        AccessControlResourceType.RevenueAnalytics,
        AccessControlLevel.Editor
    )
    const isReadOnly = !!accessDeniedReason
                ...
                <div className="space-y-4">
                    {isReadOnly && (
                        <LemonBanner type="warning" className="text-sm">
                            <strong>Read only:</strong> {accessDeniedReason} Ask a workspace admin to grant you Editor
                            access to Revenue analytics to add or change events.
                        </LemonBanner>
                    )}
                    {!originalEvent && !isReadOnly && (
                        <LemonBanner type="info" className="text-sm">
                            <strong>How it works:</strong> Txlemetry will track this event and use these properties for
                    ...
                    <div className="space-y-1">
                        ...
                        {currentEvent?.eventName && !currentEvent?.revenueProperty && (
                            <LemonBanner type="info" className="text-xs">
                                ...
                                and set its type to <code>Numeric</code>, then reopen this modal.
                            </LemonBanner>
                        )}
                    </div>
            ...
            <LemonModal.Footer>
                <div className="flex justify-end gap-2">
                    ...
                    <LemonButton
                        type="primary"
                        onClick={handleSave}
                        disabledReason={accessDeniedReason || saveEventsDisabledReason}
                    >
