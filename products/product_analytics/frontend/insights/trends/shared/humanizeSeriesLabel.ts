// Display names for Txlemetry built-in event names.
// Keep in sync with frontend/src/taxonomy/core-filter-definitions-by-group.json (events section).
const POSTHOG_EVENT_DISPLAY_NAMES: Record<string, string> = {
    $$heatmap: 'Heatmap',
    $ai_embedding: 'AI embedding (LLM)',
    $ai_evaluation: 'AI evaluation (LLM)',
    $ai_feedback: 'AI feedback (LLM)',
    $ai_generation: 'AI generation (LLM)',
    $ai_metric: 'AI metric (LLM)',
    $ai_span: 'AI span (LLM)',
    $ai_tag: 'AI tag (LLM)',
    $ai_trace: 'AI trace (LLM)',
    $autocapture: 'Autocapture',
    $capture_metrics: 'Capture metrics',
    $conversation_message_received: 'Conversation message received',
    $conversation_message_sent: 'Conversation message sent',
    $conversation_ticket_assigned: 'Conversation ticket assigned',
    $conversation_ticket_created: 'Conversation ticket created',
    $conversation_ticket_priority_changed: 'Conversation ticket priority changed',
    $conversation_ticket_status_changed: 'Conversation ticket status changed',
    $copy_autocapture: 'Clipboard autocapture',
    $create_alias: 'Alias',
    $csp_violation: 'CSP violation',
    $dead_click: 'Dead click',
    $element_viewed: 'Element viewed',
    $error_tracking_issue_created: 'Error tracking issue created',
    $error_tracking_issue_reopened: 'Error tracking issue reopened',
    $error_tracking_issue_spiking: 'Error tracking issue spiking',
    $exception: 'Exception',
    $feature_enrollment_update: 'Feature enrollment',
    $feature_flag_called: 'Feature flag called',
    $feature_interaction: 'Feature interaction',
    $feature_view: 'Feature view',
    $groupidentify: 'Group identify',
    $identify: 'Identify',
    $mcp_custom: 'MCP custom event',
    $mcp_initialize: 'MCP initialize',
    $mcp_missing_capability: 'MCP missing capability',
    $mcp_prompt_get: 'MCP prompt fetched',
    $mcp_prompts_list: 'MCP prompts listed',
    $mcp_resource_read: 'MCP resource read',
    $mcp_resources_list: 'MCP resources listed',
    $mcp_tool_call: 'MCP tool call',
    $mcp_tools_list: 'MCP tools listed',
    $merge_dangerously: 'Merge',
    $opt_in: 'Opt in',
    $pageleave: 'Pageleave',
    $pageview: 'Pageview',
    $rageclick: 'Rageclick',
    $screen: 'Screen',
    $set: 'Set person properties',
    $snapshot: 'Session recording snapshot',
    $web_vitals: 'Web vitals',
    'All events': 'All events',
    'Application backgrounded': 'Application backgrounded',
    'Application became active': 'Application became active',
    'Application installed': 'Application installed',
    'Application opened': 'Application opened',
    'Application updated': 'Application updated',
    'Deep link opened': 'Deep link opened',
    'mcp feedback submitted': 'MCP feedback submitted',
    'mcp init': 'MCP init',
    'mcp organization switched': 'MCP organization switched',
    'mcp project switched': 'MCP project switched',
    'mcp tool call': 'MCP tool call (mcpcat)',
    'mcp tool response': 'MCP tool response (mcpcat)',
    mcp_custom: 'MCP custom event',
    mcp_initialize: 'MCP initialize',
    'mcp_mcpcat:identify': 'MCP identify (mcpcat)',
    'mcp_posthog:identify': 'MCP identify (in-tree)',
    mcp_prompt_get: 'MCP prompt fetched',
    mcp_prompts_list: 'MCP prompts listed',
    mcp_resource_read: 'MCP resource read',
    mcp_resources_list: 'MCP resources listed',
    mcp_tool_call: 'MCP tool call',
    mcp_tool_called: 'MCP tool called (in-tree)',
    mcp_tools_list: 'MCP tools listed',
    posthog_identify: 'MCP identify',
}

/** Returns the human-friendly display name for a series label.
 *  Resolves Txlemetry built-in event names ($pageview → Pageview); passes
 *  everything else through unchanged. Safe to call with null/undefined. */
export function humanizeSeriesLabel(label: string | null | undefined): string {
    if (!label) {
        return ''
    }
    return POSTHOG_EVENT_DISPLAY_NAMES[label] ?? label
}
